 ...
 * AH-SK01
 */
function buildGarmentCode(prefix: string, manualCode: string): string {
...
export async function createGarment(req: Request, res: Response) {
  try {
    const { client_id, code, description, size, color, barcode, value } =
      req.body;
    ...
    if (!isNonEmptyString(client_id) || !isNonEmptyString(code)) {
    ...
    if (!isNonEmptyString(client.code_prefix)) {
      return res.status(400).json({
        ok: false,
        message: "El cliente no tiene un prefijo de código configurado",
      });
    }
    ...
    const finalCode = buildGarmentCode(client.code_prefix, code);
    ...
    if (existingCode) {
      return res.status(409).json({
        ok: false,
        message: `Ya existe una prenda con el código ${finalCode}`,
      });
    }
...
export async function updateGarment(req: Request, res: Response) {
  ...
  try {
    ...
    if (!isNonEmptyString(client.code_prefix)) {
      ...
      return res.status(400).json({
        ok: false,
        message: "El cliente no tiene un prefijo configurado",
      });
    }
    ...
    if (existingCode) {
      ...
      return res.status(409).json({
        ok: false,
        message: `Ya existe otra prenda con el código ${finalCode}`,
      });
    }
    ...
    const oldValue = Number(garment.value);
    ...
    return res.status(500).json({
      ok: false,
      message: "Error actualizando prenda",
    });
  }
}
...
export async function deactivateGarment(req: Request, res: Response) {
  try {
    ...
    return res.status(500).json({
      ok: false,
      message: "Error desactivando prenda",
    });
  }
}
