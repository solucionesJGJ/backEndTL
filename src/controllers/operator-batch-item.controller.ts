 ...
 */
function calculateBatchItemValues(garmentValue: unknown, quantitySent: number) {
...
export async function addBatchItem(req: Request, res: Response) {
  ...
  try {
    ...
    const { garment_id, quantity_sent, quantity_received, notes } = req.body;
     ...
     */
    const validation = await assertBatchEditableByClient(
      batchId,
      req,
      transaction,
    );
    ...
    const finalQuantityReceived = isClientOperator(req)
      ? 0
     ...
     */
    const { unitValue, calculatedTotal } = calculateBatchItemValues(
      garment.value,
      finalQuantitySent,
    );
...
export async function updateBatchItem(req: Request, res: Response) {
  ...
  try {
     ...
     * en item.unit_value.
     */
    const frozenUnitValue = Number(item.unit_value || 0);
     ...
     */
    const calculatedTotal = frozenUnitValue * newQuantitySent;
