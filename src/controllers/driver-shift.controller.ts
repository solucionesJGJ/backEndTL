...
import {
    DRIVER_CHECKLIST,
} from "../constant/driver-checklist.js";

import {
    generateDriverShiftPdf,
} from "../services/driver-shift-pdf.service.js";
import fs from "fs";
...
async function assertDriverRole(
    userId: string,
) {
    const user =
        await User.findByPk(
    ...
    if (
        ...
        return {
            ...
            status: 403,
            message:
                "El usuario no tiene permisos de transportista",
            user: null,
        };
    }
...
export async function getCurrentDriverShift(
    ...
    try {
        ...
        return res.status(500).json({
            ok: false,
            message:
                "Error obteniendo jornada activa",
        });
    }
}
...
export async function getDriverShiftHistory(
    ...
    try {
        ...
        return res.status(500).json({
            ok: false,
            message:
                "Error obteniendo historial de jornadas",
        });
    }
}
...
export async function startDriverShift(
    ...
    try {
        ...
        if (
            !Number.isInteger(
                parsedInitialMileage,
            )
            ||
            parsedInitialMileage < 0
        ) {
            await transaction.rollback();
            ...
            return res.status(400).json({
                ok: false,
                message:
                    "El kilometraje inicial debe ser un entero mayor o igual a 0",
            });
        }
        ...
        if (activeShift) {
            ...
            return res.status(409).json({
                ok: false,
                message:
                    "Ya existe una jornada activa para este transportista",
            });
        }
        ...
        const incomingChecks =
            checklist as IncomingChecklistItem[];
        ...
        for (
            ...
            if (
                ...
                return res.status(400).json({
                    ok: false,
                    message:
                        `Falta responder el checklist ${definition.code}`,
                });
            }
        }
        ...
        for (
            ...
            if (
                ...
                return res.status(400).json({
                    ok: false,
                    message:
                        `No puede iniciar jornada: ${definition.label}`,
                });
            }
        }
        ...
        if (!createdShift) {
            return res.status(500).json({
                ok: false,
                message:
                    "La jornada fue creada, pero no pudo recuperarse",
            });
        }
    ...
    try {
        ...
        if (
            ...
            return res.status(400).json({
                ok: false,
                message:
                    "El kilometraje final debe ser un entero mayor o igual a 0",
            });
        }
    ...
    try {
        ...
        if (
            ...
            return res.status(403).json({
                ok: false,
                message:
                    "No tienes permisos para descargar este comprobante",
            });
        }
        ...
        return res.status(500).json({
            ok: false,
            message:
                "Error descargando comprobante de jornada",
        });
    }
}
...
export async function getAllDriverShifts(
    ...
    try {
        ...
        return res.status(500).json({
            ok: false,
            message:
                "Error obteniendo jornadas de transportistas",
        });
    }
}